...
if __name__ == "__main__":
    ...
    model = Net(timesteps=30, hidden=128, beta=0.9).to(device)
    ...
    val_loader = DataLoader(val_data, batch_size=args.batch_size, shuffle=False, num_workers=int(os.cpu_count()-2))

    model = train(model, train_loader, val_loader, criterion, optimizer, scheduler, args)

    # Save your model for the last epoch
    torch.save(model.state_dict(), f"model_last_epoch{args.num_epochs}.pth")
