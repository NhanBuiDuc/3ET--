...
def train_nengo_epoch(model, sim, out_p, train_loader, args):
    total_loss = 0.0
    total_p_corr_all = {f'p{p}_all':0 for p in args.pixel_tolerances}
    total_p_error_all  = {f'error_all':0}  # averaged euclidean distance
    total_samples_all, total_sample_p_error_all  = 0, 0
    for inputs, targets in train_loader:
        # Convert inputs and targets to TensorFlow tensors
        inputs = inputs.view(inputs.shape[0], inputs.shape[1], -1)
        inputs_tf = tf.convert_to_tensor(inputs.numpy(), dtype=tf.float32)
        targets_tf = tf.convert_to_tensor(targets.numpy(), dtype=tf.float32)
        sim.fit(inputs_tf, {out_p: targets_tf})
